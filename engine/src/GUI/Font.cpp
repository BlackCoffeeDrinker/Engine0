...
const Font::GlyphCollection *Font::GlyphCollection::FindInRange(uint16_t ch) const {
  if (ch >= start && ch < end) return this;
...
void Font::Render(const std::string_view &text, Color fg, Color bg, Painter &painter, const RectT<uint16_t> &rect) {
  Vec2D<uint16_t> current_positon = rect.origin;
  const auto max_size = rect.To();

  for (const auto &ch: text) {
    if (ch == '\n') {
      current_positon.y += _height;
      current_positon.x = rect.origin.x;
      continue;
    }
    ...
    // Line ending
    if (current_positon.x + CharLength(ch) > max_size.x) {
      current_positon.y += _height;
      current_positon.x = rect.origin.x;
    }

    // Out of bmp space
    if (current_positon.y + _height > max_size.y) {
      return;
    }

    current_positon.x += RenderChar(ch, fg, bg, painter, current_positon);
  }
}
...
FontMonochrome::FontMonochrome() {
  _height = 8;
  _glyphs = std::make_unique<GlyphCollection>();
  _glyphs->start = ascii_default.start;
  _glyphs->end = ascii_default.end;
  _glyphs->glyphs = ascii_default.glyphs;
  _glyphs->next = nullptr;
}
...
uint16_t FontMonochrome::CharLength(uint16_t ch) const {
  return FindGlyph(ch).width;
}

uint16_t FontMonochrome::RenderChar(std::string_view::value_type ch, Color fg, Color bg, Painter &painter, const Vec2D<uint16_t> &position) {
  auto p = FindGlyph(ch);

  // Background
  if (bg.red != 0 || bg.green != 0 || bg.blue != 0) {
    painter.SetBrushColor(bg);
    painter.SetNoPen();
    painter.DrawRect({position, {p.width, p.height}});
  }

  painter.SetPenSolid(1, fg);
  for (uint16_t y = 0; y < p.height; ++y) {
    for (uint16_t x = 0; x < p.width; ++x) {
      if (p.dat[y] & (0x80 >> x)) {
        painter.DrawPoint({static_cast<uint16_t>(position.x + x),
                           static_cast<uint16_t>(position.y + y)});
      }
    }
  }
