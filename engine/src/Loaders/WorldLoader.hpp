...
class WorldLoader : public ResourceLoader {
  ...
  std::error_code ParseSet(Stream &stream, size_t layerIndex, const std::unique_ptr<Map> &map);
  ...
  std::error_code HandleLayerData(std::string_view key, std::string_view value);
